...
import {
  FiHome, FiFileText, FiAlertCircle,
        ...
        <nav className="flex-1 py-4 overflow-y-auto scrollbar-thin">
          {navItems.map(({ to, icon: Icon, label }) => (
            ...
            >
              ...
              <span className={`text-sm font-medium whitespace-nowrap transition-opacity duration-200 ${open ? 'opacity-100' : 'opacity-0 lg:opacity-0'}`}>
                {label}
              </span>
            </NavLink>
          ))}
          ...
          <div className={`flex items-center gap-3 px-2 py-2 ${!open && 'justify-center'}`}>
            ...
            {open && (
              <div className="overflow-hidden">
                <p className="text-xs font-semibold text-slate-700 dark:text-white truncate">Citizen User</p>
                <p className="text-xs text-slate-400 truncate">citizen@gov.in</p>
              </div>
            )}
          </div>
        </div>
